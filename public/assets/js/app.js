...
let productStickyObserver;
let productStickyCleanup;
function bindProductSticky(target) { productStickyObserver?.disconnect(); productStickyCleanup?.(); const sticky = target.querySelector(".product-sticky-cta"); const realCta = target.querySelector(".product-purchase .product-actions .button-primary"); if (!sticky || !realCta || !window.matchMedia("(max-width: 699px)").matches) return; const updateVisibility = () => { const realRect = realCta.getBoundingClientRect(); const stickyRect = sticky.getBoundingClientRect(); const overlaps = realRect.bottom > stickyRect.top && realRect.top < stickyRect.bottom; sticky.classList.toggle("is-hidden", overlaps); }; const handleScroll = () => requestAnimationFrame(updateVisibility); window.addEventListener("scroll", handleScroll, { passive: true }); window.addEventListener("resize", updateVisibility); productStickyCleanup = () => { window.removeEventListener("scroll", handleScroll); window.removeEventListener("resize", updateVisibility); }; if ("IntersectionObserver" in window) { productStickyObserver = new IntersectionObserver(updateVisibility, { threshold: [0, 0.01, 1] }); productStickyObserver.observe(realCta); } updateVisibility(); }
function setupProduct() { const product = findProduct(new URLSearchParams(location.search).get("id")); const target = document.getElementById("product-detail"); if (!product) { target.innerHTML = `<p>${t("product.notFound")}</p>`; document.title = t("titles.product"); return; } document.title = `${product.nom} | ${t("siteName")}`; const checkoutUrl = `commande.html?id=${encodeURIComponent(product.id)}`; const actions = available(product) ? `<div class="product-actions"><a class="button button-primary" href="${checkoutUrl}">Acheter maintenant</a><button class="button button-outline" type="button" data-add-cart="${product.id}">${t("product.add")}</button></div>` : `<span class="button button-disabled">${t("product.unavailable")}</span>`; const status = available(product) ? "" : `<span class="availability">${t("product.comingSoon")}</span>`; const paymentLogos = [{ name: "Orange Money", file: "orange-money.svg" }, { name: "Moov Money", file: "moov-money.png" }, { name: "Wave", file: "wave.svg" }, { name: "Visa", file: "visa.svg" }, { name: "Mastercard", file: "mastercard.svg" }, { name: "Bitcoin", file: "bitcoin.svg" }]; const paymentMethods = `<div class="payment-methods" aria-label="Moyens de paiement disponibles"><p>Moyens de paiement disponibles</p><div class="payment-method-list">${paymentLogos.map((payment) => `<span class="payment-method"><img src="assets/payments/${payment.file}" alt="${payment.name}"></span>`).join("")}</div></div>`; const purchasePanel = `<aside class="product-purchase" aria-label="Zone d'achat"><p class="purchase-label">Aperçu de votre achat</p><p class="product-price">${formatPrice(product.prix)}</p>${status}${actions}${available(product) ? paymentMethods : ""}<div class="purchase-links"><a href="contact.html">Contact</a><span aria-hidden="true">|</span><a href="contact.html">Signaler</a></div><details class="purchase-help"><summary>Comment acheter ?</summary><p>Choisissez votre devise, puis cliquez sur Acheter maintenant pour continuer vers le paiement sécurisé.</p></details></aside>`; const stickyAction = available(product) ? `<div class="product-sticky-cta"><strong>${formatPrice(product.prix)}</strong><a class="button button-primary" href="${checkoutUrl}">Acheter maintenant</a></div>` : ""; target.innerHTML = `<div class="product-heading"><h1>${escapeHtml(product.nom)}</h1></div><div class="product-left"><div class="product-visual"><img src="${escapeHtml(productImage(product))}" alt="${escapeHtml(product.nom)}"></div><div class="rich-description">${product.description}</div></div>${purchasePanel}${stickyAction}`; bindCartButtons(target); bindProductSticky(target); }
...
function setupCheckout() {
  ...
  const phoneCountryCodeInput = document.getElementById("phone-country-code");
  const normalizePhoneCountryCode = () => {
    phoneCountryCodeInput.value = phoneCountryCodeInput.value.trim().toUpperCase();
  };
  phoneCountryCodeInput.addEventListener("input", normalizePhoneCountryCode);
  phoneCountryCodeInput.addEventListener("change", normalizePhoneCountryCode);
